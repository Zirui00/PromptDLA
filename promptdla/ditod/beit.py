...
if args.dataset == "doclaynet":
    prompt_dicts = {'government_tenders': 0, 'laws_and_regulations': 1, 'financial_reports': 2, 'patents': 3,
                    'manuals': 4, 'scientific_articles': 5}
elif args.dataset == "m6doc":
    prompt_dicts = {"Scientific articles": 0, "Textbooks": 1, "Books(photo)": 2, "Test papers": 3, "Magazines": 4,
                    "Newspapers": 5, "Notes": 6}
else:
    prompt_dicts = {"publaynet": 0, "doclaynet": 1}


# prompt_dicts = {'bosi': 0, 'gaomian': 1, 'hasake': 2, 'laowo': 3, 'tuerqi': 4, 'yindi': 5, 'yuenan': 6}
#
# prompt_dicts = {"docbank": 0}
...
class BEiT(nn.Module):
    ...
    def __init__(self,
        ...
        if args.dataset == "doclaynet":
            self.prompt_token = nn.Parameter(torch.zeros(1, 6, 512), requires_grad=False)
        elif args.dataset == "m6doc":
            self.prompt_token = nn.Parameter(torch.zeros(1, 7, 512), requires_grad=False)
        else:
            self.prompt_token = nn.Parameter(torch.zeros(1, 2, 512), requires_grad=False)
        with open(args.prompt_file, "r") as jsonfile:
            prompt_tensor = json.load(jsonfile)
        for key in prompt_dicts.keys():
            self.prompt_token.data[0][prompt_dicts[key]] = torch.tensor(prompt_tensor[key])

        self.prompt_linear_project = nn.ModuleList(depth * [nn.Linear(512, 768)])
    ...
    def forward_features(self, x_total):
        if args.use_prompt == 1:
            x = x_total[0]
            doc_categories = x_total[1]
            B, C, H, W = x.shape
            ...
            for i, blk in enumerate(self.blocks):
                linear_project = self.prompt_linear_project[i]
                prompts = []
                for category in doc_categories:
                    prompt = self.prompt_token[:, prompt_dicts[category]:prompt_dicts[category] + 1, :]
                    prompt_in = prompt.view(1, 512)
                    prompt_out = linear_project(prompt_in).view(1, 1, 768)
                ...
                if len(prompts) == 1:
                    ...
                    for j in range(1, len(prompts)):
                        prompt_tokens = torch.cat((prompt_tokens, prompts[j]), dim=0)

                if self.pos_embed is not None:
                    prompt_tokens = prompt_tokens + self.pos_embed[:, :1, :]
                word_tokens = torch.cat((cls_tokens, prompt_tokens), dim=1)
                x = torch.cat((word_tokens, x[:, -Hp * Wp:, :]), dim=1)
